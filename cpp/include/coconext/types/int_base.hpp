...
class IntBackend {
  ...
  private:
    ...
    static constexpr StorageType truncate(StorageType val) {
        constexpr unsigned bits_in_val = sizeof(StorageType) * 8;

        if constexpr (_numBits >= bits_in_val) {
            return val;
        } else {
            constexpr StorageType mask = (StorageType(1) << _numBits) - 1;

            StorageType unsigned_val = val & mask;

            if constexpr (_is_signed) {
                constexpr unsigned sign_shift = _numBits - 1;
  ...
  public:
    // Literal Int
    constexpr IntBackend()
        requires(std::integral<StorageType>)
    = default;

    constexpr IntBackend(StorageType val)
        requires(std::integral<StorageType>)
        : _storage(truncate(val)) {}
