...
class CountIterator {
  public:
    ...
    using difference_type =
        std::conditional_t<(sizeof(IntT) > sizeof(std::ptrdiff_t)), IntT, std::ptrdiff_t>;
  ...
  public:
    ...
    constexpr CountIterator(CountIterator const&) noexcept = default;
    constexpr CountIterator& operator=(CountIterator const&) noexcept = default;
  ...
  public:
    ...
    constexpr CountIterator operator+(difference_type n) const noexcept {
        return direction_ == Direction::TO ? CountIterator(current_ + n, direction_)
    ...
    constexpr difference_type operator-(CountIterator const& other) const noexcept {
        assert(direction_ == other.direction_ && "Iterators from different ranges");
    ...
    friend constexpr auto operator<=>(
        CountIterator const& lhs, CountIterator const& rhs
    ) noexcept {
        assert(lhs.direction_ == rhs.direction_ && "Iterators from different ranges");
        // For DOWNTO, current_ decreases as we advance, so the natural ordering
        // on current_ is inverted relative to iteration order.
        return lhs.direction_ == Direction::TO ? (lhs.current_ <=> rhs.current_)
    ...
    friend constexpr CountIterator operator+(
        difference_type n, CountIterator const& it
    ) noexcept {
        return it + n;
    }
