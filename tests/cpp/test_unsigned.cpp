...
TEST(TestUnsigned, ConstructInRange) {
    Unsigned<4> a(10);
...
TEST(TestDynUnsigned, Construct) {
    DynUnsigned a(10, 4);
    EXPECT_EQ(a.value(), 10U);
    EXPECT_EQ(a.width(), 4U);
    // Length-only sugar produces DOWNTO range.
    EXPECT_EQ(a.range(), (Range{3, Direction::DOWNTO, 0}));
}

TEST(TestDynUnsigned, ConstructFromExplicitRange) {
    DynUnsigned a(10, Range(15, Direction::DOWNTO, 12));
    EXPECT_EQ(a.value(), 10U);
    EXPECT_EQ(a.width(), 4U);
    EXPECT_EQ(a.range(), (Range{15, Direction::DOWNTO, 12}));
}

TEST(TestDynUnsigned, ConstructOverflowThrows) {
    EXPECT_THROW(DynUnsigned(16, 4), std::out_of_range);
}

TEST(TestDynUnsigned, ConstructNegativeThrows) {
    EXPECT_THROW(DynUnsigned(-1, 4), std::out_of_range);
}

TEST(TestDynUnsigned, BadWidthThrows) {
    EXPECT_THROW(DynUnsigned(0, 0), std::out_of_range);
    EXPECT_THROW(DynUnsigned(0, 65), std::out_of_range);
}

TEST(TestDynUnsigned, ArithmeticWrapAndMaxWidth) {
    auto c = DynUnsigned(15, 4) + DynUnsigned(250, 8);
    EXPECT_EQ(c.width(), 8U);
    EXPECT_EQ(c.value(), 9U);  // 265 mod 256
}

TEST(TestDynUnsigned, SubWraps) {
    EXPECT_EQ((DynUnsigned(0, 4) - DynUnsigned(1, 4)).value(), 15U);
}

TEST(TestDynUnsigned, DivModByZeroThrows) {
    EXPECT_THROW(DynUnsigned(1, 8) / DynUnsigned(0, 8), std::domain_error);
    EXPECT_THROW(DynUnsigned(1, 8) % DynUnsigned(0, 8), std::domain_error);
}

TEST(TestDynUnsigned, Bitwise) {
    EXPECT_EQ((DynUnsigned(0b1100, 4) & DynUnsigned(0b1010, 4)).value(), 0b1000U);
    EXPECT_EQ((~DynUnsigned(0, 4)).value(), 15U);
}

TEST(TestDynUnsigned, Shifts) {
    EXPECT_EQ((DynUnsigned(0b0011, 4) << 2).value(), 0b1100U);
    EXPECT_EQ((DynUnsigned(15, 4) << 64).value(), 0U);
    EXPECT_THROW(DynUnsigned(1, 4) << -1, std::invalid_argument);
}

TEST(TestDynUnsigned, CompoundAssign) {
    DynUnsigned a(10, 4);
    a += DynUnsigned(7, 4);
    EXPECT_EQ(a.value(), 1U);
    EXPECT_EQ(a.width(), 4U);
...
TEST(TestDynUnsigned, Comparisons) {
    EXPECT_TRUE(DynUnsigned(3, 4) == DynUnsigned(3, 8));
    EXPECT_TRUE(DynUnsigned(3, 4) < DynUnsigned(4, 4));
}

TEST(TestDynUnsigned, ToNative) {
    EXPECT_EQ(DynUnsigned(200, 8).to<int>(), 200);
    EXPECT_THROW((void)DynUnsigned(40000, 16).to<uint8_t>(), std::out_of_range);
}

TEST(TestDynUnsigned, Formatter) { EXPECT_EQ(std::format("{}", DynUnsigned(42, 8)), "42"); }
// LCOV_EXCL_BR_STOP
